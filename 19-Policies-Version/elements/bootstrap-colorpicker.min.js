...
strVal=strVal.toLowerCase().trim();return(strVal=='transparent')||(strVal.match(/#?00000000/))||(strVal.match(/(rgba|hsla)\(0,0,0,0?\.?0\)/))},rgbaIsTransparent:function(rgba){return((rgba.r==0)&&(rgba.g==0)&&(rgba.b==0)&&(rgba.a==0))},setColor:function(strVal){strVal=strVal.toLowerCase().trim();if(strVal){if(this.isTransparent(strVal)){this.value={h:0,s:0,b:0,a:0}}else{this.value=this.stringToHSB(strVal)||{h:0,s:0,b:0,a:1}}}},stringToHSB:function(strVal){strVal=strVal.toLowerCase();var that=this,result=!1;$.each(this.stringParsers,function(i,parser){var match=parser.re.exec(strVal),values=match&&parser.parse.apply(that,[match]),format=parser.format||'rgba';if(values){if(format.match(/hsla?/)){result=that.RGBtoHSB.apply(that,that.HSLtoRGB.apply(that,values))}else{result=that.RGBtoHSB.apply(that,values)}
that.origFormat=format;return!1}
return!0});return result},setHue:function(h){this.value.h=1-h},setSaturation:function(s){this.value.s=s},setBrightness:function(b){this.value.b=1-b},setAlpha:function(a){this.value.a=parseInt((1-a)*100,10)/100},toRGB:function(h,s,b,a){if(!h){h=this.value.h;s=this.value.s;b=this.value.b}
...
break;case 'alias':return this.toAlias()||this.toHex();default:{return!1}
...
return!1}};var defaults={horizontal:!1,inline:!1,color:!1,format:!1,input:'.form-control_CP',container:!1,component:'.add-on, .input-group-addon_CP',sliders:{saturation:{maxLeft:100,maxTop:100,callLeft:'setSaturation',callTop:'setBrightness'},hue:{maxLeft:0,maxTop:100,callLeft:!1,callTop:'setHue'},alpha:{maxLeft:0,maxTop:100,callLeft:!1,callTop:'setAlpha'}},slidersHorz:{saturation:{maxLeft:100,maxTop:100,callLeft:'setSaturation',callTop:'setBrightness'},hue:{maxLeft:100,maxTop:0,callLeft:'setHue',callTop:!1},alpha:{maxLeft:100,maxTop:0,callLeft:'setAlpha',callTop:!1}},template:'<div class="colorpicker dropdown-menu">'+'<div class="colorpicker-saturation"><i><b></b></i></div>'+'<div class="colorpicker-hue"><i></i></div>'+'<div class="colorpicker-alpha"><i></i></div>'+'<div class="colorpicker-color"><div /></div>'+'</div>'};var Colorpicker=function(element,options){this.element=$(element).addClass('colorpicker-element');this.options=$.extend({},defaults,this.element.data(),options);this.component=this.options.component;this.component=(this.component!==!1)?this.element.find(this.component):!1;if(this.component&&(this.component.length===0)){this.component=!1}
...
this.color=new Color(this.options.color!==!1?this.options.color:this.getValue());this.format=this.options.format!==!1?this.options.format:this.color.origFormat;this.picker=$(this.options.template);if(this.options.inline){this.picker.addClass('colorpicker-inline colorpicker-visible')}else{this.picker.addClass('colorpicker-hidden')}
...
this.picker.on('mousedown.colorpicker touchstart.colorpicker',$.proxy(this.mousedown,this));this.picker.appendTo(this.container?this.container:$('body'));if(this.input!==!1){this.input.on({'keyup.colorpicker':$.proxy(this.keyup,this)});if(this.component===!1){this.element.on({'focus.colorpicker':$.proxy(this.show,this)})}
...
this.update();$($.proxy(function(){this.element.trigger('create')},this))};Colorpicker.Color=Color;Colorpicker.prototype={constructor:Colorpicker,destroy:function(){this.picker.remove();this.element.removeData('colorpicker').off('.colorpicker');if(this.input!==!1){this.input.off('.colorpicker')}
...
return val},setValue:function(val){this.color=new Color(val);this.update();this.element.trigger({type:'changeColor',color:this.color,value:val})},getValue:function(defaultValue){defaultValue=(defaultValue===undefined)?'#000000':defaultValue;var val;if(this.hasInput()){val=this.input.val()}else{val=this.element.data('color')}
...
return!1},currentSlider:null,mousePointer:{left:0,top:0},mousedown:function(e){if(!e.pageX&&!e.pageY&&e.originalEvent){e.pageX=e.originalEvent.touches[0].pageX;e.pageY=e.originalEvent.touches[0].pageY}
...
return!1},mousemove:function(e){if(!e.pageX&&!e.pageY&&e.originalEvent){e.pageX=e.originalEvent.touches[0].pageX;e.pageY=e.originalEvent.touches[0].pageY}
...
return $returnValue};$.fn.colorpicker.constructor=Colorpicker}))